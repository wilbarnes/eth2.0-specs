...
def ssz_leaves(obj, root=1):
    ...
    if is_bottom_layer_kind(typ):
        ...
        if isinstance(obj, Container):
            elems = [getattr(obj, field) for field in typ.get_fields()]
        else:
            elems = [obj[i] for i in range(len(obj))]
        starting_index = base * next_power_of_two(chunk_count(typ))
        for i, elem in enumerate(elems):
            o = {**o, **ssz_leaves(elem, root=starting_index + i)}
        return {**o, **empty_sisters(starting_index, len(elems), chunk_count(typ))}
...
class SSZPartial():
    ...
    def append_or_pop(self, appending, value):
        ...
        if new_chunk_count != old_chunk_count:
            for k, v in empty_sisters(start_pos, old_chunk_count, chunk_count(self.typ)).items():
                del self.objects[k]
        if not appending:
            if issubclass(elem_type, BasicValue):
